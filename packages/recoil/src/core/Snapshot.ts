...
} from '../../../shared/src/util/Recoil_Memoize';
...
export class Snapshot {
    ...
    getNodes_UNSTABLE = (
        ...
        return opt?.isInitialized == null
            ...
            : opt.isInitialized === true
                ...
                : filterIterable(
                    recoilValues.values(),
                    (recoilValue) => !knownAtoms.has(recoilValue.key) && !knownSelectors.has(recoilValue.key),
                );
    };
    ...
    getInfo_UNSTABLE = <T>(recoilValue: RecoilValue<T>): RecoilValueInfo<T> => {
        this.checkRefCount_INTERNAL();
        return peekNodeInfo(this._store, this._store.getState().currentTree, recoilValue.key);
    };
...
export function cloneStoreState(
    ...
    return {
        ...
        nodeCleanupFunctions: new Map(
            mapIterable(storeState.nodeCleanupFunctions.entries(), ([key]: [string, any]) => [
                key,
                () => { },
            ]),
        ),
...
export function freshSnapshot(initializeState?: (mutableSnapshot: MutableSnapshot) => void): Snapshot {
