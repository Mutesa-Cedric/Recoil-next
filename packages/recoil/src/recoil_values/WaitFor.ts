...
function unwrapDependencies(
  ...
  return Array.isArray(dependencies)
    ...
    : Object.getOwnPropertyNames(dependencies).map(
        key =>
          (dependencies as {[key: string]: RecoilValueReadOnly<unknown>})[key],
      );
}
...
export const waitForNone: <T extends RecoilValues>(
  ...
  T extends ReadonlyArray<RecoilValueReadOnly<any>> | Readonly<{}>
    ? {
...
export const noWait: <T>(
  ...
  key: '__noWait',
  get:
    (dependency: RecoilValue<unknown>) =>
    ({get}) => {
      try {
