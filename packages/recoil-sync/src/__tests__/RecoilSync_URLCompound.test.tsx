...
import {encodeURL, expectURL} from '../__test_utils__/MockURLSerialization';
import {
...
test('Upgrade item ID', async () => {
  ...
  history.replaceState(
    null,
    '',
    encodeURL([[{part: 'queryParams'}, {old_key: 'OLD'}]]),
  );
...
test('One atom to many items', async () => {
  ...
  history.replaceState(
    null,
    '',
    encodeURL([[loc, {obj: {foo: 123, bar: 456}}]]),
  );
...
test('One atom to atom family', async () => {
  ...
  const [AtomFoo, atomFooControls] = ComponentThatReadsAndWritesAtom(
    atomFamilyFoo('foo'),
  );
  const [AtomBar, atomBarControls] = ComponentThatReadsAndWritesAtom(
    atomFamilyFoo('bar'),
  );
  const container = renderElements(
    React.createElement(RecoilURLSyncJSON, {
