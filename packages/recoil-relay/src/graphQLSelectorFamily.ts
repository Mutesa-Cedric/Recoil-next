...
> {
  ...
  mutations?: {
    ...
    variables: (
      data: T,
    ) => TVariables | null | ((param: P) => TVariables | null);
  };
}
...
>({
  ...
  >({
    ...
    ): ReadonlyArray<AtomEffect<InternalAtomData<P, TData, T>>> => {
      const effects: AtomEffect<InternalAtomData<P, TData, T>>[] = [
        ...
        (query as any).params?.operationKind === 'query'
          ? (graphQLQueryEffect({
              ...
              variables: vars,
              query,
              mapResponse: (response: TData) =>
                ({source: 'remote', response}) as const,
