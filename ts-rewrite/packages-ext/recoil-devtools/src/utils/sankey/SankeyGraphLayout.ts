/**
 * TypeScript port of SankeyGraphLayout.ts
 */
...
import {updateVisibility} from './SankeyGraph';
import * as d3Array from 'd3-array';
// d3-collection was merged into d3-array in d3 v6+
...
type FlowLayoutOptions = PositionLayoutOptions & {
...
type ButterflyLayoutOptions = PositionLayoutOptions & {
...
// Utility functions
const sortAsc = <T>(array: T[], accessor: (item: T) => number): T[] =>
  array.sort((a, b) => accessor(a) - accessor(b));
const sortDesc = <T>(array: T[], accessor: (item: T) => number): T[] =>
...
function limitNodes<N, L>(
  ...
  const consideredLinks: Set<Link<any, any>> = new Set(nextLinks);

  function addNode(node: Node<N, L>) {
    if (!nodesToAdd) {
      return;
    }
    nodesSet.add(node);
...
const flowGraphLayout = <N, L>(
  layoutOptions: Partial<FlowLayoutOptions> = FLOW_LAYOUT_DEFAULT,
): LayoutFunction<N, L> => {
...
const butterflyGraphLayout = <N, L>(
  focalNodeKey: Key,
  layoutOptions: Partial<ButterflyLayoutOptions> = BUTTERFLY_LAYOUT_DEFAULT,
): LayoutFunction<N, L> => {
  const layoutOptionsConfig = {
