/**
 * TypeScript port of Recoil_Retention.js
 */
...
import { cleanUpNode } from './FunctionalCore';
import { deleteNodeConfigIfPossible, getNode } from './Node';
...
import someSet from '../../../shared/src/util/Recoil_someSet';
...
function releaseNode(store: Store, treeState: TreeState, node: NodeKey): void {
...
function nodesRetainedByZone(
    storeState: ReturnType<Store['getState']>,
    zone: RetentionZone,
): Set<NodeKey> {
    return storeState.retention.nodesRetainedByZone.get(zone) ?? emptySet;
}

export function zonesThatCouldRetainNode(node: NodeKey): Array<RetentionZone> {
    const nodeConfig = getNode(node);
    const retainedBy = nodeConfig.retainedBy;
    if (
...
export function updateRetainCountToZero(store: Store, retainable: Retainable): void {
    if (!gkx('recoil_memory_managament_2020')) {
        return;
    }
    const map = store.getState().retention.referenceCounts;
...
export function releaseScheduledRetainablesNow(store: Store): void {
    if (!gkx('recoil_memory_managament_2020')) {
        return;
    }
    const state = store.getState();
...
export function retainedByOptionWithDefault(
    r?: RetainedBy,
): RetainedBy {
