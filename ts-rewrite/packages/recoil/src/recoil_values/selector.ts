...
import { isRecoilValue, RecoilState, RecoilValue, RecoilValueReadOnly, } from '../core/RecoilValue';
...
import { recoilCallback, RecoilCallbackInterface } from '../hooks/useRecoilCallback';
...
import { GetRecoilValue, ResetRecoilState, SetRecoilState, ValueOrUpdater, } from './callbackTypes';
...
export type GetCallback<T> = <Args extends ReadonlyArray<unknown>, Return>(
    fn: (callbackInterface: SelectorCallbackInterface<T>) => (...args: Args) => Return,
) => (...args: Args) => Return;
...
export function selector<T>(
    ...
    function getLoadableFromCacheAndUpdateDeps(
        ...
        try {
            ...
            throw err(
                `Problem with cache lookup for selector "${key}": ${(error as Error)
                    .message}`,
            );
        }
    ...
    function getInProgressExecutionInfo(
        ...
        const pendingExecutions = Array.from(concatIterables([
    ...
    function setCache(
        ...
        try {
            ...
            throw err(
                `Problem with setting cache for selector "${key}": ${(error as Error)
                    .message}`,
            );
        }
    }
    ...
    function selectorPeek(
        ...
        const cachedLoadable = state.atomValues.get(key) as
            | Loadable<T>
            | undefined;
        if (cachedLoadable != null) {
            return cachedLoadable;
