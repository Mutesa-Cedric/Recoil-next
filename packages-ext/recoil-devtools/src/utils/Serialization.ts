// TypeScript port of Serialization.js
...
function serialize(
  ...
  let serialized: SerializedValue | null = null;
  if (item === null) {
    serialized = {t: SerializedValueType.null};
  } else if (item === undefined) {
    serialized = {t: SerializedValueType.undefined};
  } else if (Array.isArray(item)) {
    const {iterable, exceeds} = getExceedingItems<any>(item, maxItems);
    ...
    const {iterable, exceeds} = getExceedingItems<any>(
      Array.from(item),
      maxItems,
    );
    serialized = maybeAddExceeds(
    ...
    const {iterable, exceeds} = getExceedingItems<[any, any]>(
    ...
    const {iterable, exceeds} = getExceedingItems<[string, any]>(
      Object.entries(item),
      maxItems,
    );
    serialized = maybeAddExceeds(
...
function getExceedingItems<T>(
  items: readonly T[],
  maxItems: number,
): {exceeds: number; iterable: readonly T[]} {
 ...
 */
// TODO: Handle promises and immutable
function deserialize(item: SerializedValue | null | undefined): any {
  if (item == null) {
    return null;
  }
...
function formatForDiff(item: SerializedValue | null | undefined): any {
  if (item == null) {
    return 'undefined';
  }
  const {v: value, t: type} = item;
...
export {serialize, deserialize, formatForDiff, SerializedValueType};
