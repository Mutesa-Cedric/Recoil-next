/**
 * TypeScript port of SerializationTest.ts
 * Recoil DevTools browser extension.
 */

import {SerializedValueType, deserialize, serialize} from '../Serialization';
...
describe('Preparing objects to be sent via postMessage', () => {
  ...
  it('maps', () => {
    const map = new Map<string, string | number | Set<number>>([
      ['test', 1234],
      ['other', 'serialized'],
    ]);
    ...
    map.set('nested', new Set<number>([9, 2, 3]));
  ...
  it('sets', () => {
    const set = new Set<string | number | Set<number>>(['test', 1234]);
  ...
  it('errors', () => {
    ...
    expect(
      ...
      t: SerializedValueType.error,
      v: 'Error: test 123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678 (...)',
    });
  });
});
