...
import {select} from './CV2_D3';
import {generateGraph} from './SankeyGraph';
import * as d3Interpolate from 'd3-interpolate';
import * as d3Scale from 'd3-scale';
import React, {useLayoutEffect, useMemo, useRef} from 'react';

export type LinkData<L> = {
  readonly data: readonly L[];
  readonly getLinkValue: (link: L) => number;
  readonly getLinkSourceKey: (link: L) => Key | null | undefined;
  readonly getLinkTargetKey: (link: L) => Key | null | undefined;
};
...
export type NodeData<N> = {
  readonly data: readonly N[];
  readonly getNodeKey: (node: N) => Key;
  readonly getNodeName: (node: N) => string;
  readonly getNodeValue?: (node: N) => number; // If not provided, node value is based on max of incoming or outgoing links
};

type Layout<N, L> = {
  readonly graph: Graph<N, L>;
  readonly depthDomain: [number, number];
  readonly positionDomain: [number, number];
};

export type LayoutFunction<N, L> = (params: {
  graph: Graph<N, L>;
...
type Props<N, L> = {
  ...
  readonly layout: LayoutFunction<N, L>;
  readonly links: LinkData<L>; // Input link data
  readonly nodes?: NodeData<N>; // If not provided, nodes are derived from link data
  readonly getNodeTooltip?: (node: Node<N, L>) => string;
  readonly nodeStyles?: Styles<Node<N, L>>;
  readonly nodeClass?: string | ((node: Node<N, L>) => string);
  readonly nodeLabelStyles?: Styles<Node<N, L>>;
  readonly nodeLabelClass?: string | ((node: Node<N, L>) => string);
  readonly nodeEvents?: Events<Node<N, L>>;
  // Height of a node in pixels (x-axis in horizontal, y-axis in vertical orientation)
  readonly nodeThickness?: number;
  // Align node labels
  readonly nodeLabelAlignment?: 'inward' | 'right';
  // Curvature for curved link paths (range 0-1, defaults to 0.5)
  readonly getLinkTooltip?: (link: Link<L, N>) => string;
  readonly linkColor: string | ((link: Link<L, N>) => string);
  readonly linkStyles?: Styles<Link<L, N>>;
  readonly linkClass?: string | ((link: Link<L, N>) => string);
  readonly linkEvents?: Events<Link<L, N>>;
  ...
  readonly linkThickPathThreshold?: number;
  readonly valueFormatter?: (value: number) => string; // Formatter for values in default tooltips
  readonly animationDurationMS?: number; // Animation duration in ms
...
const functor = <T extends any>(x: T | ((arg: any) => T)): ((arg: any) => T) =>
  typeof x === 'function' ? (x as (arg: any) => T) : () => x;

/**
 * @explorer-desc
 * Sankey Visualization inspired from https://drarmstr.github.io/chartcollection/examples/#sankey/
 */
function Sankey<N extends any, L extends any>({
  height,
  width,
  margin = 10,
  orientation,
  ...
  useLayoutEffect(() => {
    ...
    // Render Nodes
    const nodeDepth = (n: Node<N, L>) => depth(n.depth);
    const nodePosition = (n: Node<N, L>) => breadth(n.position);
    const nodeWidth = (n: Node<N, L>) => breadth(n.value);
    nodesSelection.attr({
    ...
    const labelAlignRight = (n: Node<N, L>) =>
