/**
 * TypeScript port of SankeyGraph.ts
 */
...
export type Node<N, L> = {
  ...
  visible: boolean;
  position: number;
  readonly sourceLinks: Link<L, N>[];
  readonly targetLinks: Link<L, N>[];
  linkWeight: number;
};

export type Graph<N, L> = {
  readonly links: readonly Link<L, N>[];
  readonly nodes: readonly Node<N, L>[];
};
...
function generateGraph<N, L>({
  ...
  for (const node of nodes) {
    ...
    function detectBackedge(node: any) {
