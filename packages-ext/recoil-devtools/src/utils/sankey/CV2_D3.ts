...
export type Events<T> = {
  readonly [key: string]: null | ((data: T) => void);
};
...
class Selection<T> {
  all: D3Selection<any, any, any, any>;
  new: D3Selection<any, any, any, any>;
  old: D3Selection<any, any, any, any>;
  update: D3Selection<any, any, any, any>;
  options: Options;
  ...
  bind<U>(
    selector: string,
    data: readonly U[],
    key: (data: U) => string | number,
  ): Selection<U> {
    const [tag, className] = selector.split('.');

    const updateSelection = this.all.selectAll(selector).data(data, key as any);
  ...
  attr(attrs?: Attributes<T>): Selection<T> {
    if (attrs != null) {
      function attr(selection: any, attrs: Attributes<T>) {
  ...
  style(styles?: Styles<T>): Selection<T> {
    if (styles != null) {
      for (const [style, value] of objectEntries(styles)) {
        (this.all as any).style(style, value ?? null);
      }
    }
    return this;
  }
  ...
  on(events?: Events<T>): Selection<T> {
    if (events != null) {
      for (const [event, handler] of objectEntries(events)) {
        (this.all as any).on(event, handler ?? null);
      }
    }
    return this;
  }
  ...
  text(str: string | ((data: T) => string)): Selection<T> {
    this.all.text(str);
    return this;
  }

  // Set the inner HTML for the elements in the selection.
  html(str: string | ((data: T) => string)): Selection<T> {
    this.all.html(str);
    return this;
  }
}
