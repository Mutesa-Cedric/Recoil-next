...
import err from '../util/Recoil_err';
...
export type RecoilInterface = {
  ...
  getRecoilState: <T>(recoilState: RecoilState<T>) => [T, SetterOrUpdater<T>];
...
function useRecoilValueLoadable_SYNC_EXTERNAL_STORE<T>(
  ...
  if (useSyncExternalStore === undefined) {
    throw new Error(
      'useSyncExternalStore is not available in this version of React',
    );
  }

  return useSyncExternalStore(subscribe, getMemoizedSnapshot, () => ssrSnapshot)
    .loadable;
}
