...
import {isSSR} from '../util/Recoil_Environment';
...
import usePrevious from '../util/Recoil_usePrevious';
    ...
    const wrappedCallback = (store: Store) => {
      try {
        ...
        if (
          ...
          console.warn(
            'Snapshot already released in transaction subscription, skipping',
          );
          return;
        }
        throw error;
  ...
  const [snapshot, setSnapshot] = useState(() => {
    try {
      ...
      if (
        ...
        console.warn(
          'Snapshot already released during initial state, creating fresh snapshot',
        );
        return cloneSnapshot(storeRef.current);
      }
      throw error;
    ...
    useCallback((store: Store) => {
      try {
        ...
        if (
          ...
          console.warn(
            'Snapshot already released during transaction subscription, skipping update',
          );
          return;
        }
        throw error;
