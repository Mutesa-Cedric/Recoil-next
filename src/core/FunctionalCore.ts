...
import lazyProxy from '../util/Recoil_lazyProxy';
...
export function peekNodeInfo<T>(
  ...
  return lazyProxy({type} as Partial<RecoilValueInfo<T>>, {
...
export function getDownstreamNodes(
  ...
  while ((key = visiting.pop()) !== undefined) {
    ...
    const subs =
      graph.nodeToNodeSubscriptions.get(key) ??
      (emptySet as ReadonlySet<NodeKey>);
