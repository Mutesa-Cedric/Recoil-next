...
export abstract class BaseLoadable<T> {
  ...
  promiseOrThrow(): Promise<T> {
    throw err(
      `Loadable expected promise, but in "${(this as any).state}" state`,
    );
  }
...
export class ValueLoadable<T> extends BaseLoadable<T> {
  ...
  map<S>(map: (value: T) => Promise<S> | Loadable<S> | S): Loadable<S> {
    try {
      ...
      return isPromise(e)
        ? loadableWithPromise(
            (e as Promise<any>).then(() =>
              this.map(map).toPromise(),
            ) as Promise<S>,
          )
        : loadableWithError(e);
    }
...
export function loadableLoading<T>(): Readonly<LoadingLoadable<T>> {
  return Object.freeze(
    new LoadingLoadable(new Promise(() => {}) as Promise<T>),
  );
}

type UnwrapLoadables<Loadables> = {
  [K in keyof Loadables]: Loadables[K] extends Loadable<infer V> ? V : never;
};
