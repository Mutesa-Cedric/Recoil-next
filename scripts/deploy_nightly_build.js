...
function updatePackageVersion(packagePath, nightlyVersion) {
  ...
  if (packageJson.dependencies) {
    for (const [dep, version] of Object.entries(packageJson.dependencies)) {
      if (dep.startsWith('recoil') || dep.startsWith('refine') || dep.startsWith('@recoiljs/')) {
...
function deployNightly() {
  ...
  try {
    ...
    for (const packagePath of DEPLOYABLE_PACKAGES) {
      ...
      try {
        execSync(`pnpm publish --tag ${NIGHTLY_TAG} --access public --no-git-checks`, {
          cwd: path.join(projectRoot, packagePath),
          stdio: 'inherit'
        });
        console.log(`✅ Published ${packageInfo.name}@${nightlyVersion}`);
      } catch (error) {
        console.error(`❌ Failed to publish ${packageInfo.name}:`, error.message);
        throw error;
      }
    }
    ...
    for (const [packagePath, originalVersion] of Object.entries(originalVersions)) {
      try {
